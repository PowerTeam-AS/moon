...
if [ ! -e "$TARGET_FILE" ]; then
  # /etc/passwd가 없는 경우는 매우 치명적인 에러
  state="100" # 에러 상태 코드
  content="${TARGET_FILE}_Not_Found"
else
  # 2. 파일 정보 추출
  # %U: 소유자, %a: 8진수 권한 (예: 644)
  read -r F_OWNER F_PERM < <(stat -c "%U %a" "$TARGET_FILE")
  ...
  # 644 이하의 의미: Group(중간)과 Other(끝)에 Write(2) 권한이 없어야 하며,
  # 일반적으로 6(rw-), 7(rwx) 처럼 4(r--) 보다 큰 값이 오면 취약으로 간주
  # (실행 권한 x(1)도 /etc/passwd에는 불필요하므로 4보다 크면 취약으로 판단)
